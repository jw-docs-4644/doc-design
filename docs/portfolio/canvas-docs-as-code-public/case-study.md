...
## Overview
This project uses a docs-as-code philosophy to create course materials for the Canvas Learning Management System. It reduces the time it takes to develop a fully-fleshed out, complex Canvas course from weeks to days. Treating all course materials as code means that coding tools and approaches can make it easier to develop well-thought-out, consistent materials   

## Context

Most educators work with Canvas through its web interface, which as far as web interface go, is ... fine. But a course of even modest complexity involves interactions among course elements which can quickly become difficult to keep track of, if not completely overwhelming. The Canvas web interface does provide tools to check for broken links and accessibility problems, but no help in actually fixing the problematic course materials. Even worse, working the web interface makes it difficult to implement pedagogical changes in response to student or colleague feedback: If I update the assessment criteria in an assignment prompt, it can be extremely difficult to find and update all of the course materials (drafts, revisions, rubrics. etc.) that are affected by that change. 
...
To implement this system, I had to solve problems such as 
...
The overarching design philosophy was to make building the course "text-first and local-first." To the extent possible, I wanted to build a system that let teachers write an entire course without making any adjustments in the Canvas web interface. 

As such, the project was guided by well established docs-as-code principles:
...
- Materials are
    ...
    - structured
    - modular
    - transparent. Learning materials are kept in a small number of folders with clear labels:
        - Assignments
        - Discussions
        - Files
        - Rubrics

*Note: Quizzes present an obvious gap here. I don't use quizzes much in my teaching, and didn't have any planned for the pilot course, so I did not build any scripts or templates to handle them.*
...
I estimate that the current system is 90% there. (One exception I haven't solved yet involves setting up separate sections and assignments for grad students sections that are layered with undergraduate sections.) 

## Outcomes

The end result works much better than I had originally hoped. When I started, I expected to be able to push assignments to Canvas and then make modifications there. Thanks to the yaml headers in each piece of content, nearly every imaginable box that needs to be ticked in Canvas to make a course actually work for students can be set before publishing. Thanks to LLM-based coding agents, that metadata can easily be adjusted and made consistent across materials. 
